...
\usepackage{natbib}
\usepackage[dvips, bookmarks, backref,pdfpagelabels,colorlinks=false, linkcolor=blue, urlbordercolor={0 0 1},citebordercolor={1 0 0},pdftitle={StaMPS/MTI Manual}, pdfauthor={Andy Hooper }, pdfkeywords={PS, PSI, MT-InSAR, StaMPS}]{hyperref}
%\usepackage{url}
...
{\sffamily\Huge{\textbf{\textsc{\\}}}}
  ...
  {\sffamily\Huge{\textbf{{\\
   StaMPS/MTI  Manual\\
     ...
     \vspace{5mm}
     {\sffamily\Large{\textbf{Version 3.4}}}
\vspace{5mm}
...
%\begin{flushright}
\begin{center}
{\sffamily\textbf{Andy Hooper and David Bekaert}}\\
\vspace{4mm}
{\sffamily\textbf{5th June, 2018}}
%\end{flushright}
\end{center}
...
StaMPS/MTI is made available for non-commercial applications only. Releases can be downloaded from \url{http://homepages.see.leeds.ac.uk/~earahoo/stamps/} and github at \url{https://github.com/dbekaert/stamps}. The github webpage also allows to download the developers version which will be the most recent and un-released version.  \\
...
In addition, cite dependencies used such as ROI\_PAC \citep{Rosen2004}, DORIS \citep{Kampes2003}, GAMMA \citep{Werner2000}, ISCE \citep{rosen2012}, and or TRAIN \citep[e.g.][]{Bekaert2015,Bekaert2015a}.
...
There are two pre-processing steps before getting to the PS/MTI processing proper. The first is to focus the raw data (if required), and the second is to form interferograms from single-look complex (SLC) images. Different processors can be used together with StaMPS such as ROI\_PAC and DORIS, GAMMA, and ISCE. Further details are provided below on the support of each software.
...
\subsection{ISCE}
StaMPS is compatible with the outputs generated though the ISCE stack processor. Note that no shell scripts are included within the StaMPS distribution itself to generate ISCE interferograms. Users should refer to the contrib folder of the ISCE software for generating a stack compatible with StaMPS. 

\subsection{SNAP}
StaMPS is also compatible with the output generated by the ESA SentiNel Application Platform (SNAP) after the version 6.0 (SNAP is downloadable from 
\url{http://step.esa.int/main/download/} ). 
...
You can download from \url{https://github.com/mdelgadoblasco/snap2stamps} a set of graphs together with python wrappers which allow you to automatise the interferogram processing chain for single master interferograms compatible with StaMPS PSI.

Information about the provided functionalities and their instructions can be found in the user manual provided within the github package which already plans newer releases increasing functionality and compatibility maintenance of SNAP-StaMPS chain.

Demonstration about the ESA SNAP and StaMPS compatibility for PSI analysis has been already shown in EGU2018 and IGARSS 2018 with the work done under the title: "ESA SNAP – STAMPS INTEGRATED PROCESSING FOR SENTINEL-1 PERSISTENT SCATTERER INTERFEROMETRY" with Michael Foumelis and Jose Manuel Delgado Blasco among the authors.

When using this software package please reference \cite{Foumelis2018}

\subsubsection{Master Selection}
In order to minimize the amount of data volume produced during the interferogram formation we strongly recommend to select the minimum burst on the master image that cover the entire AOI. Hence, this step requires visual inspection of the SAR image and hence we recommend to do it by using the SNAP GUI.



\subsubsection{Slaves Splitting}

\subsubsection{Coregistration and Interferogram formation}

\subsubsection{StaMPS export}
...
\subsection{Triangle}
The Triangle program is used for Delaunay triangulation. It is only required in case users want to run stamps beyons step 7. It can be found at:\\
...
\subsection{SNAP}
Details on installation and usage of SentiNel Application Platform (SNAP) can be found at \url{http://step.esa.int/main/download/} while the additional software package for the interferometric stack computation can be found at: \url{https://github.com/mdelgadoblasco/snap2stamps/}

\chapter{Pre-processing with ROI\_PAC and DORIS} 
\label{ch:roipac_doris_IFG}
\section{Create SLCs from Raw (using ROI\_PAC)}
...
\section{Reading/cropping SLCs  from Level 1 products}
...
\subsection{\label{sec:slcread1} Option 1: Specify by latitude and longitude}
...
\subsection{\label{sec:slcread2}Option 2: Specify by line/pixel number}
...
\section{\label{sec:ifgs} Create IFGs (using DORIS)}
...
\subsection{\label{sec:calcdem}Manual DEM offset correction}
...
\subsection{Possible reasons for Doris SIGERV error}
...
\chapter{Pre-processing with SNAP}
\label{ch:SNAP_IFG}
Note that no shell scripts are included within the StaMPS distribution itself to generate SNAP interferograms. Users should refer to the snap2stamps software package for generating a stack compatible with StaMPS using the ESA SNAP software. 

The following steps are needed: 
\begin{enumerate}
\item Sort the slave images in separated folders corresponding to their acquisition time. The current scripts can handle maximum of 2 slaves per acquisition day.
\item Slave splitting (and slice assembling if needed) and anotated orbits replacement with Precise/Restituted Orbits. Current scripts allow to work with each subswath separately.
...
\item StaMPS matlab preparation routine can be now run to get StaMPS friendly dataset structure.
\end{enumerate}

The corresponding python routines for running the aforementioned steps are provided in the github snap2stamps package available at \url{https://github.com/mdelgadoblasco/snap2stamps}

\chapter{Pre-processing with ISCE}
\label{ch:isce_IFG}
...
\chapter{Pre-processing with Gamma}  
\label{ch:gamma_IFG}

\chapter{PS Processing}
First, create single master interferograms by following the pre-processing Chapter of your preferred InSAR processor, i.e. chapter ~\ref{ch:roipac_doris_IFG} for ROI\_PAC/DORIS,  chapter ~\ref{ch:isce_IFG} for ISCE or chapter ~\ref{ch:gamma_IFG} for Gamma. 
In the {\tt INSAR\_}\cc{master\_date} directory run
 \cs{mt\_prep}  for ROI\_PAC/DORIS,  \cs{mt\_prep\_gamma} for Gamma,   \cs{mt\_prep\_isce} for ISCE, and \cs{mt\_prep\_snap} for SNAP. For example:  \\
...
First, create single master interferograms by following the pre-processing chapter of the InSAR processor applied, i.e. chapter ~\ref{ch:roipac_doris_IFG} for ROI\_PAC/DORIS, chapter ~\ref{ch:isce_IFG} for ISCE, chapter ~\ref{ch:SNAP_IFG} for SNAP, or chapter ~\ref{ch:gamma_IFG} for Gamma. Note, the steps below are for DORIS and Gamma. The processing steps for ISCE up to the \cs{mt\_prep\_isce} are differnt and detailed in chapter ~\ref{ch:isce_IFG}.
