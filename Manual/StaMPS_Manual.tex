...
%\begin{flushright}
\begin{center}
{\sffamily\textbf{Andy Hooper, Karsten Spaans, David Bekaert, Miguel Caro Cuenca, Mahmut Ar{\i}kan, Anneleen Oyen}}\\
\vspace{4mm}
{\sffamily\textbf{24th November, 2010}}
%\end{flushright}
\end{center}
...
StaMPS/MTI is made available for non-commercial applications only and can be downloaded from \url{http://radar.tudelft.nl/~ahooper/stamps}
...
Currently, scripts exist for (reading and cropping single look complex (SLC) products from ERS, Envisat, RADARSAT-1 and TerraSAR-X satellites.
...
This will create a directory named {\tt SLC} in the current directory, with a subdirectory for each image named \cc{yyyymmdd}. Within each subdirectory, symbolic links will be created to the slc files (and leader/volume files if they exist). For ERS and RADARSAT-1 data the link names must be {\tt DAT\_01.001}, {\tt  LEA\_01.001}, and {\tt VDF\_DAT.001}. For Envisat  data the link name must be {\tt image.slc}. For TerraSAR-X data, the link names must be {\tt image.slc} and {\tt leader.xml}. \\
...
%add some tips in the future
 ...
 For a single master approach, oversampling shows significant improvement in PS density especially in the rural areas (see \citet{Arikan2010}).
...
with the following line (substituting the correct heading, to the nearest degree is fine):\\
 ...
 \cs{tsx\_extract\_heading} (you must have python installed to run this)
...
\begin{small}
 ...
 You can view a baseline plot for all interferobrams not dropped with \cs{plot\_sb\_baselines}.
  \end{small}
