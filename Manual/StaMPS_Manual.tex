...
{\sffamily\Huge{\textbf{\textsc{\\}}}}
  ...
  {\sffamily\Huge{\textbf{{\\
   StaMPS/MTI  Manual\\
     ...
     \vspace{5mm}
     {\sffamily\Large{\textbf{Version 3.3}}}
\vspace{5mm}
...
%\begin{flushright}
\begin{center}
{\sffamily\textbf{Andy Hooper, David Bekaert, Karsten Spaans}}\\
\vspace{4mm}
{\sffamily\textbf{1th September, 2013}}
%\end{flushright}
\end{center}
...
StaMPS/MTI is made available for non-commercial applications only and can be downloaded from \textbf{\url{http://homepages.see.leeds.ac.uk/~earahoo/stamps/}}
...
There are two pre-processing steps before getting to the PS/MTI processing proper. The first is to focus the raw data (if required), and the second is to form interferograms from single-look complex (SLC) images. ROI\_PAC is used for the focusing and Doris for interferogram formation. If starting with SLC images, rather than raw data, the focusing step is skipped and the images are imported directly into Doris. Currently, support is provided for processing ERS, Envisat and ALOS data, if starting with raw data, and for  ERS, Envisat, RADARSAT-1/2, TerraSAR-X and \textbf{COSMO-SkyMed} data, if starting with SLCs.
...
%StaMPS is designed to run under the tcsh shell (you can usually invoke this by typing \cs{tcsh} from whatever shell you are within). Alternatively adapt {\tt StaMPS\_CONFIG} for your favourite shell.
 ...
 Install StaMPS/MTI: \\
 \textbf{\cs{ tar -xvf StaMPS\_v3.3.tar}\\
 cd StaMPS\_v3.3/src\\
 make\\
 make install}
...
\section{Triangle}
 ...
 The optimisation routines of snaphu are used by the 3-D unwrapping code and can be downloaded from:\\
  \textbf{\url{http://www.stanford.edu/group/radar/softwareandlinks/sw/snaphu/}}.
...
This will create a directory named {\tt SLC} in the \cc{processing\_path} directory, with a subdirectory for each image named \cc{yyyymmdd}. Within each subdirectory, symbolic links will be created to the data files. For ERS data the link names are  {\tt IMAGERY}\cc{yyymmdd} and {\tt SARLEADER}\cc{yyyymmdd} (\underline{4 digit, not 2 digit, years}), and for Envisat and ALOS data, the link names are the same as the original names. 
 ...
 \textbf{For Envisat wide-swath (DORIS orbits):}\\
\textbf{ \cs{step\_slc\_envi\_ws}}\\  
 For ALOS data:\\
  \cs{step\_slc\_alos}\\
...
%\end{list}
 ...
 %\cs{remake\_slcs}\\
 %\cs{cd} \cc{master\_date}\\
 \cs{step\_slc\_alos}, \cs{step\_slc\_ers},  \cs{step\_slc\_ers\_envi}, \textbf{\cs{step\_slc\_envi\_vor}}, \cs{step\_slc\_envi} (attach \cs{\_V2.2} suffix if necessary), or \textbf{\cs{step\_slc\_envi\_ws}}.\\
 %\cs{multilook} \cc{master\_date}\cs{.slc} \cc{5612 }
...
\cs{make\_slcs\_alos}, \cs{make\_slcs\_ers}, \cs{make\_slcs\_ers\_envi}, \textbf{\cs{make\_slcs\_envi\_vor}},
 \cs{make\_slcs\_envi} (attach \cs{\_V2.2} suffix if necessary), or  \textbf{\cs{make\_slcs\_envi\_ws}}. This will create SLCs for all directories listed in {\tt make\_slcs.list}, which by default contains all data directories except the master.
...
{\em If you created SLCs with ROI\_PAC using raw data, skip Section: \ref{sec:slcread}.}

\section{\label{sec:slcread}Reading and Cropping}
Currently, scripts exist for (reading and cropping single look complex (SLC) products from ERS, Envisat, RADARSAT-1/\textbf{2}, TerraSAR-X and \textbf{COSMO-SkyMed} satellites.
...
This will create a directory named {\tt SLC} in the current directory, with a subdirectory for each image named \cc{yyyymmdd}. Within each subdirectory, symbolic links will be created to the slc files (and leader/volume files if they exist). For ERS and RADARSAT-1 data the link names must be {\tt DAT\_01.001}, {\tt  LEA\_01.001}, and {\tt VDF\_DAT.001}. For Envisat and \textbf{COSMO-SkyMed} data the link name must be {\tt image.slc}. For TerraSAR-X and \textbf{RADARSAT-2} data, the link names must be {\tt image.slc} and {\tt leader.xml}. \\
...
Choose a master based on minimising perpendicular, Doppler and temporal baselines (see \citet{HooperA2007a}). Substitute your master date in the format {\tt yyyymmdd} wherever \cc{master\_date} appears below.
 ...
 \cs{cd} \cc{master\_date}\\
 \cs{step\_read\_whole\_}\cc{XXX} (where \cc{XXX} is `\cs{ERS}', `\cs{Envisat}', `\cs{RSAT}', \textbf{`\cs{RSAT2}'}, `\cs{TSX}', or \textbf{`\cs{CSK}'})
...
\begin{tabular}{p{4.5cm}p{1.8cm}p{8.5cm}}
\textbf{Parameter Name} &	\textbf{Default} & \textbf{Description}\\
{\tt unwrap\_method} &	 {\tt `3D'} &  Unwrapping method. \textbf{****MORE options need to be included and explained, ref to a paper?****} \\
...
\textbf{For small baselines processing, some of the parameters have different default values compared to PS processing:}\\
\footnotesize
\begin{tabular}{p{4.5cm}p{1.8cm}p{8.5cm}}
\textbf{Parameter Name} &	\textbf{Default} & \textbf{Description}\\
{\tt select\_method} &
...
\textbf{\url{http://www.see.leeds.ac.uk/people/a.hooper}}
