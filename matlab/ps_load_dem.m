...
%if fid>0
  ...
  dem_lat1=dem_lat2 - (dem_length-1)*dem_posting; % lower right corner
  dem_lon2=dem_lon1 + (dem_width-1)*dem_posting; % lower right corner
  ...
  if dem_lon1<lon1
      j1=round((lon1-dem_lon1)/dem_posting)+1;
  end
  if dem_lon2>lon2
      j2=round((lon2-dem_lon1)/dem_posting)+1;
  end
  if dem_lat1<lat1
      i2=round((dem_lat2-lat1)/dem_posting)+1;
  end
  if dem_lat2>lat2
      i1=round((dem_lat2-lat2)/dem_posting)+1;
  end

  dem=dem(i1:i2,j1:j2);
  dem_lon1=dem_lon1+(j1-1)*dem_posting;
  dem_lat1=dem_lat2-(i2-1)*dem_posting;
  dem_length=i2-i1+1;
  dem_width=j2-j1+1;
  ...
  x_posting=posting(1);
  y_posting=posting(2);
  x_offset=(ll0(1)-dem_lon1)/dem_posting*x_posting;
  y_offset=(ll0(2)-dem_lat1)/dem_posting*y_posting;
  ...
  if max(posting)<min_posting
    ...
    dem_lon=dem_lon1+(1/scale_factor_x-1)/2*dem_posting;
    dem_lat=dem_lat1+(1/scale_factor_y-1)/2*dem_posting;
    dem_posting_lon=dem_posting*scale_factor_x;
    dem_posting_lat=dem_posting*scale_factor_y;
  else
    dem_lon=dem_lon1;
    dem_lat=dem_lat1;
    dem_posting_lon=dem_posting;
    dem_posting_lat=dem_posting;
